...
async function getClaudeDescriptionFromImage(
  ...
  try {
    ...
    if (imageUrl) finalImage = await downloadImage(imageUrl);
      ...
      finalImage = getFileTypeAndData(image.image);
    }
...
async function getGoogleGeminiDescriptionFromImage(
  ...
  try {
    ...
    if (imageUrl) {
      ...
      imageData = getFileTypeAndData(image.image);
      finalImage = {
        inlineData: {
          data: imageData.base64Data,
          mimeType: imageData.fileType,
    ...
    console.log(finalImage);
    const result = await model.generateContent([promptText, finalImage]);
