...
async function getClaudeDescriptionFromImage(
  ...
  try {
    ...
    return parseAltTextAndLongDescription(msg.content[0].text, prompt);
  } catch (error) {
    console.error(
...
async function getGoogleGeminiDescriptionFromImage(
  ...
  try {
    ...
    return parseAltTextAndLongDescription(content, prompt);
  } catch (error) {
    console.error(
 ...
 */
function parseAltTextAndLongDescription(content, prompt) {
  try {
    switch (prompt) {
      case "metadata":
      ...
      case "sentiment analysis":
        const [sentimentTags, reasoning] = content.split("REASONING:");
      return {
        sentimentTags: sentimentTags.replace("SENTIMENT TAGS:", "").trim(),
        reasoning: reasoning.trim(),
      };
      case "tags":
        return {
          artTags: content.replace("ART TAGS:", "").trim(),
        };
      default:
        return content;
    }
